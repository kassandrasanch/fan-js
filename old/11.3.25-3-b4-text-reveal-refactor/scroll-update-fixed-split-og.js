...
//   const isPageHeader   = wrapper.getAttribute("element") === "page-header";
//   const isRippleGradient = wrapper.getAttribute("data-element") === "ripple-gradient";
//   if (isRippleGradient) { initVideoTextScrub(wrapper); return; }
...
//   // --- MASTER (scrubbed + snapped) ---
//   const spacerPerStep = 1;            // virtual distance between steps
...
//       trigger: wrapper,
//       start: "top top",
//       end: () => `+=${steps * 1400}`, // visual feel; match your old value
//       pin: true,
//       scrub: 1,
//       // markers: true,
//       // Simple equal-interval snapping: nearest step
//       snap: {
//         snapTo: (value) => gsap.utils.snap(1 / steps, value),
...
//   // --- Page header pre-fade ---
//   let fansVideo, logoVideo;
//   if (isPageHeader) {
//     fansVideo = wrapper.querySelector(".n_bg-video--fans");
//     logoVideo = wrapper.querySelector(".n_bg-video--logo");
//     gsap.set(logoVideo, { opacity: 0, y: 0 });
//     if (fansVideo) {
//       const preFade = gsap.timeline()
//         .to(fansVideo, { opacity: 0, duration: 0.8, ease: "power2.out" })
//         .to(fansVideo, { zIndex: "-10", pointerEvents: "none" });
//       master.add(preFade, 0);
//     }
//   }

//   // -- helper plays one block (NOT scrubbed), then resumes master --
//   function playBlock(block, index) {
//     block.classList.add("active");
//     gsap.set(block, { zIndex: 2 });

//     const special  = block.dataset.animateSpecial;
//     const hasLogos = block.classList.contains("n_logo-reveal__block");

//     const tl = gsap.timeline({
//       onComplete: () => {
//         block.classList.remove("active");
//         gsap.set(block, { zIndex: 0 });
//         master.play(); // release the pause; user must scroll again for next step
//       }
//     });

//     // --- specials ---
//     if (special === "anton")           { tl.add(animateAnton(block)); return tl.play(0); }
//     if (special === "instrument")      { tl.add(animateInstrument(block)); return tl.play(0); }
//     if (special === "inter")           { tl.add(animateInter(block)); return tl.play(0); }
//     if (special === "bk-switch--white"){ tl.add(animateBk(block)); return tl.play(0); }
//     if (special === "like-this")       { tl.add(animateLikeThis(block, wrapper)); return tl.play(0); }
//     if (special === "logomark")        { tl.add(animateLogoMark(block, wrapper)); return tl.play(0); }

//     // --- normal text: all lines of all headings move together ---
//     const headings = Array.from(block.querySelectorAll(".n_text-reveal__heading"));
//     const splits   = headings.map(h => splitHeading(h)); // -> { words, lines }
//     const lines    = splits.flatMap(s => s.lines);
//     const words    = splits.flatMap(s => s.words);

//     tl.add(prepareLines(lines, words))              // prep
//       .add(animateLinesIn(lines, words, endColor)); // in (non-scrubbed)

//     // optional fast-svg inside this block
//     if (special === "fast-svg") {
//       const section = block.closest(".n_text-reveal-section");
//       const fastSVG = section ? section.querySelector(".fast-svg") : null;
//       if (fastSVG) tl.add(fastSVGAni(fastSVG));
//     }

//     if (hasLogos) {
//       // logo rows
//       const logoLists = [...block.querySelectorAll(".n_logo-row__logos")];
//       const logoTl = gsap.timeline();
//       logoLists.forEach((logoList, idx) => {
//         const parentWidth = logoList.parentElement.offsetWidth;
//         const innerWidth  = logoList.offsetWidth;
//         const maxMove     = Math.max(innerWidth - parentWidth, 0);
...
//       const notLast = index !== (blocks.length - 1);
//       if (notLast) {
//         const inner = block.querySelector(".n_logo-reveal__inner");
//         if (inner) {
//           tl.to(inner, {
//             opacity: 0,
//             scale: 0.95,
...
//       }
//     } else {
//       tl.add(animateLinesOut(lines)); // out (non-scrubbed)
//     }

//     tl.play(0);
//   }

//   // Build steps: spacer (gives distance to snap), then pause to play block
//   blocks.forEach((block, i) => {
//     master.add(`step${i}`);
//     master.to({}, { duration: spacerPerStep });
//     master.addPause(() => playBlock(block, i));
//   });

//   // Optional: precise snap using the labels we just added (uncomment if you add uneven timings)
//   // const snapPoints = blocks.map((_, i) => master.labels[`step${i}`] / master.duration());
//   // master.scrollTrigger.vars.snap = {
//   //   snapTo: (value) => gsap.utils.closest(snapPoints, value),
//   //   duration: { min: 0.15, max: 0.5 },
//   //   ease: "power3.out"
//   // };
//   // master.scrollTrigger.refresh();

//   // Optional final page-header logo fade
//   if (isPageHeader && logoVideo) {
//     master.addPause(() => {
//       gsap.timeline({
//         onComplete: () => master.play()
//       }).to(logoVideo, { opacity: 1, duration: 2, ease: "power2.out" });
//     });
//   }
// }
...
function scrollSlider(slider) {
    ...
    // const dots = slider.querySelectorAll(".dot-wrapper");
    // dots.forEach((dot) => {
    //   // prep
    //   console.log("dot found:", dot);
    //   gsap.set(dot, { clipPath: "circle(0% at 50% 100%)" });

    //   // pick a trigger inside the same slide (fallback to the slide itself)
    //   const hostSlide = dot.closest(".section-banner, .n_section-header, .section-split") || dot.parentElement;
    //   const triggerEl = hostSlide.querySelector(".dot-reveal-section .text-color-black") || hostSlide;

    //   // make the dot animate while the container timeline (tl) plays
    //   gsap.timeline({
    //     scrollTrigger: {
    //       trigger: triggerEl,
    //       start: "top center",
    ...
    //       scrub: 1,
    //       // markers: true,
    //       containerAnimation: tl   // <-- key line
    //     }
    //   }).to(dot, {
