...
// Helper: split each .reveal-line into .line-item__inner words
...
function prepareWords(words) {
  // Each blocks words start hidden below and fan red:
  gsap.set(words, {
    yPercent: 100,
    opacity: 0,
    color: "#E10600",
...
function animateWordsIn(words, endColor) {
  return gsap.timeline()
    // 1 from red/offscreen → final position/visible **and** retain red
    .fromTo(words, {
      yPercent: 100,
      opacity: 0,
      color: "#E10600"
      ...
      yPercent: 0,
      opacity: 1,
      // we don’t touch `color` yet, so words stay red
      duration: 1,
      ease: "power4.out",
      stagger: 0.15
    })
    // 2 then tween red → endColor
    .to(words, {
      color: endColor,
      duration: 1,
      stagger: 0.15
    }, 0); // `<0` would wait for the first tween to finish; `0` overlaps both
}
...
function initVideoTextScrub(wrapper) {
  ...
  video.addEventListener('loadedmetadata', () => {
    ...
    blocks.slice(0, 2).forEach((block, blkIdx) => {
      block.querySelectorAll('.n_text-reveal__heading').forEach((h1, hIdx) => {
        const words = splitTextIntoWords(h1);
        prepareWords(words);

        // only mark “last” when you’re on the *second* block’s last heading
        const isLastHeading = blkIdx === 1 && hIdx === block.querySelectorAll(
          '.n_text-reveal__heading').length - 1;

        introTl.add(
          createWordTimeline(words, "#ffffff", block, false, isLastHeading)
        );
      });
    });
...
function animateInstrument(block) {
  ...
  //   loop: true,
  //   autoplay: true,
  //   path: 'https://cdn.prod.website-files.com/67b4c8583d604cb6c2fc9a62/685005cd5d4e1a5d8f65c24d_2025_06_12_EASCI_Fanatics_TypeInstrument_02.json'
  // });
...
//       // https://cdn.prod.website-files.com/67b4c8583d604cb6c2fc9a62/685005cd5d4e1a5d8f65c24d_2025_06_12_EASCI_Fanatics_TypeInstrument_02.json
...
//       specialTl.add(animateWordsIn(words, "#ffffff"));
...
//   // 2.4) (If isPageHeader && there is a logoVideo, we do one more “fade in the logo”
//   const totalDuration = masterTl.duration();
//   if (isPageHeader && logoVideo) {
//     const lastLine = blocks[blocks.length - 1];
//     const lastWords = lastLine.querySelectorAll('text-reveal__Word');
...
function initFanSansLottie() {
  ...
  let lottieAnimation = lottie.loadAnimation({
    ...
    autoplay: false,
    // first bad
    // path: "https://cdn.prod.website-files.com/67b4c8583d604cb6c2fc9a62/6854313915112e5b15a22404_2025_06_18_EASCI_Fanatics_RoundFanatics_02.json"
    // WORKING
  ...
  // lottieAnimation.addEventListener("data_failed", () => console.error("❌ data_failed"));
  // lottieAnimation.addEventListener("data_ready", () => console.log("✅ data_ready"));
  // lottieAnimation.addEventListener("DOMLoaded", () => console.log("✅ DOMLoaded — totalFrames:",
  //   lottieAnimation
  //   .totalFrames));
  // lottieAnimation.addEventListener("error", (e) => console.error("❌ lottie error", e));
  ...
  lottieAnimation.addEventListener("DOMLoaded", () => {
    ...
    //     trigger: section,
    //     start: "top top",
    //     end: "+=2000", // must match your Lottie scrub end
    //     scrub: true,
    //   }
    // });
  })
...
function scrollAccordion(accordionRoot) {
  //  Find elements relative to this accordion instance
  ...
  //  Hide initially for scroll-reveal
  // gsap.set(wrapper, { autoAlpha: 0 });

  //  ScrollTrigger to fade in
  ScrollTrigger.create({
  ...
  //  Set uniform heights for all panels (for smooth scroll/pin effect)
  function setUniformHeights() {
  ...
  //  Pin and scrub animation (scroll-jacked accordion)
  ScrollTrigger.create({
    id: accordionRoot.id,
    trigger: accordionRoot,
    pin: true,
  ...
  //  Keyboard/Click accessibility: tab navigation!
...
//     { from: 10_000_000, to: 100_000_000 },
//     { from: 100_000_000, to: 500_000_000 },
//     { from: 500_000_000, to: 1_000_000_000 },
//   ];
//   let i = 0;
...
//   const from = 999_899;
//   const to = 999_999_999;
//   const D = 200_000; // 20 seconds

//   el.textContent = from.toLocaleString(); // reset
//   animateCountUp(el, from, to, D);
// }
...
// //   // calculate numbers‐per‐ms from your original 999,899→1,150,000 over 6000ms
// //   const rate = (1_150_000 - 999_899) / 6000;
// //   const duration = Math.ceil((to - from) / rate);

// //   el.textContent = from.toLocaleString(); // reset
// //   animateCountUp(el, from, to, duration);
// // }
...
//   const slider = document.querySelector('.n_scrolling-slider');
//   const intro = slider.querySelector('.scrolling-slider__intro');
//   const track = slider.querySelector('.scrolling-slider__track');
//   const slides = gsap.utils.toArray(track.querySelectorAll('.scrolling-slider__slide'));
//   const total = slides.length;
//   const imgs = slides.map(s => s.querySelector('.scrolling-slide__background'));

//   // 1) Prep layout & initial states
//   gsap.set(track, { display: 'flex' });
//   slides.forEach((slide, i) => {
//     gsap.set(slide, { flex: '0 0 100%', xPercent: i === 0 ? 0 : 100 });
//     // hide text until revealed
//     gsap.set(slide.querySelector('.scrolling-slide__title'), { opacity: 0, y: 20 });
//     gsap.set(slide.querySelector('.scrolling-slide__caption'), { opacity: 0, y: 20 });
//   });
//   imgs.forEach(img => gsap.set(img, { xPercent: 0, scale: 1.1 }));

//   // 2) Build scrubbed, snapped timeline
//   const tl = gsap.timeline({
//     scrollTrigger: {
//       trigger: slider,
//       pin: true,
...
//   // 3) Fade in the intro header
//   tl.fromTo(intro, { opacity: 0, yPercent: 20 }, { opacity: 1, yPercent: 0, duration: 1,
//     ease: 'power4.out' });

//   slides.forEach((slide, i) => {
//     // a) For the very first slide, just zoom out the image and fade in its text
//     if (i === 0) {
//       tl.to(imgs[0], { scale: 1, duration: 1.2, ease: 'power3.out' }, 0);
//       tl.to(slides[0].querySelector('.scrolling-slide__title'), { opacity: 1, y: 0,
//         duration: 0.6, ease: 'power2.out' }, 0.2);
//       tl.to(slides[0].querySelector('.scrolling-slide__caption'), { opacity: 1, y: 0,
//         duration: 0.6, ease: 'power2.out' }, 0.4);
//       return;
//     }
...
//       cur = slide,
//       curImg = imgs[i],
//       title = cur.querySelector('.scrolling-slide__title'),
//       caption = cur.querySelector('.scrolling-slide__caption');

//     // b) Slide the *previous* panel out to the left
//     tl.to(prev, { xPercent: -100, duration: 1, ease: 'power4.inOut' });
//     // c) Push its image out to the right in sync
//     tl.to(prevImg, { xPercent: 100, duration: 1, ease: 'power4.inOut' }, '<');

//     // d) Pull the *current* panel in from the right
//     tl.fromTo(cur, { xPercent: 100 }, { xPercent: 0, duration: 1, ease: 'power4.inOut' }, '<');
//     // e) Slide its image in from the left
//     tl.fromTo(curImg, { xPercent: -100 }, { xPercent: 0, duration: 1, ease: 'power4.inOut' },
//       '<');

//     // f) Zoom out the current image a bit
//     tl.to(curImg, { scale: 1, duration: 1.2, ease: 'power3.out' }, '<0.2');
//     // g) Fade in its title & caption
//     tl.to(title, { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' }, '<0.2');
//     tl.to(caption, { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' }, '<0.4');
//   });
// }
...
//   console.log(parentWidth)
//   const extraSpace = slider.clientWidth * 2;

//   gsap.to(track, {
//     x: `-${totalWidth - parentWidth + 200}px`,
//     ease: 'none',
//     scrollTrigger: {
//       trigger: slider,
//       start: 'top top',
//       end: `+=${(totalWidth - parentWidth) * 2}`,
//       pin: true,
//       scrub: true,
//       invalidateOnRefresh: true,
//       markers: true
