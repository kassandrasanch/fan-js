...
function resetAndPlay(vid) {
  if (!vid) return;
  // ensure metadata, then seek to 0 and play
  const doStart = () => {
    try { vid.pause(); } catch {}
...
//   // 0) PANELS = your two ripple backgrounds
//   const panels = gsap.utils.toArray(
//     wrapper.querySelectorAll('.ripple-gradient__bg-wrapper .ripple-gradient')
//   );
//   const total = panels.length; // should be 2 (red, blue)

//   // prep their starting positions like the big slider
//   gsap.set(panels, { xPercent: i => (i === 0 ? 0 : 100) }); // red on, blue off-right
//   const prevVid = panels[0]?.querySelector('video');
//   const curVid = panels[1]?.querySelector('video');
//   gsap.set([prevVid, curVid], { xPercent: 0 }); // we’ll parallax them later

//   const video = wrapper.querySelector('video'); // first found; we won’t scrub time now
//   const blocks = gsap.utils.toArray(wrapper.querySelectorAll('.n_text-reveal__block'));
//   const totalBlocks = blocks.length;

//   // if you don’t actually need to scrub video time anymore, you can skip preload/load
//   if (video) {
...
//       pin: true,
//       scrub: 1, // <- scrub like your big slider
//       // snap: { snapTo: 1/(blocks.length-1), duration: 0.3, ease: 'power1.inOut' }, // optional
//       toggleActions: 'play reverse play reverse',
//     }
//   });

//   // 1) INTRO: first two blocks (unchanged from your logic)
...
//   // 2) MAIN TEXT + PANEL SWAP ON THE 3RD HEADING
...
//     // 👉 when we hit the *third* heading (blockIndex === 2),
//     // swap the ripple panels like your big slider
//     if (blockIndex === 2 && total >= 2) {
//       videoTl.addLabel('rippleSwap');

//       // slide containers
//       videoTl.to(panels[0], { xPercent: -100, duration: 1, ease: 'power4.inOut' },
//       'rippleSwap');
//       videoTl.fromTo(panels[1], { xPercent: 100 }, { xPercent: 0, duration: 1,
//         ease: 'power4.inOut' }, 'rippleSwap');

//       // optional parallax on the <video> nodes
//       videoTl.to(prevVid, { xPercent: 50, duration: 1, ease: 'power4.inOut' }, 'rippleSwap');
//       videoTl.fromTo(curVid, { xPercent: -50 }, { xPercent: 0, duration: 1,
//         ease: 'power4.inOut' }, 'rippleSwap');

//       // 🔁 restart the correct video at the moment of swap
//       videoTl.call(() => {
//         // forward: start blue, stop red; reverse: start red, stop blue
//         const goingForward = !videoTl.reversed();
//         const inVid = goingForward ? curVid : prevVid;
//         const outVid = goingForward ? prevVid : curVid;

//         if (outVid) { try { outVid.pause();
//             outVid.currentTime = 0; } catch {} }
//         if (inVid) resetAndPlay(inVid);
//       }, null, 'rippleSwap');
//       // videoTl.addLabel('rippleSwap');

//       // // container slides (prev out left, next in from right)
//       // videoTl.to(panels[0], {
//       //   xPercent: -100,
//       //   duration: 1,
//       //   ease: 'power4.inOut'
...
//       // // optional image-like counter‑motion on the <video> elements for depth
//       // if (prevVid) {
//       //   videoTl.to(prevVid, {
//       //     xPercent: 50,
//       //     duration: 1,
...
//     // your per-heading word reveals continue as before
...
// function initVideoTextScrub(wrapper) {
//   const panels = gsap.utils.toArray(wrapper.querySelectorAll(
//     '.ripple-gradient__bg-wrapper .ripple-gradient'));
...
//         // is this the last heading *in this block*?
//         const isLastHeading = hIdx === headings.length - 1;
//         // is this the second block (so we know when to hand off to the video scrub)?
//         const isLastBlock = blkIdx === 1;

//         introTl.add(
//           createWordTimeline(
//             words,
//             "#ffffff", // your target color
//             block,
//             isLastBlock, // tells createWordTimeline if it's the final “text” block
//             isLastHeading // now true for last heading in BOTH block 0 & block 1
//           )
//         );
//       });
//     });
...
//     blocks.slice(2).forEach((block, i) => {
//       const blockIndex = i + 2; // <-- NEW: get the real index in `blocks`
//       const headings = block.querySelectorAll('.n_text-reveal__heading');
//       headings.forEach((h1, headingIndex) => {
//         const words = splitTextIntoWords(h1);
//         prepareWords(words);

//         // only true if this is the very last block in the whole set
//         const isLastBlock = blockIndex === totalBlocks - 1;
//         const isLastHeading = headingIndex === headings.length - 1;
//         // only animate out on non-final blocks' last headings
//         const passLastHeading = isLastHeading && !isLastBlock;
...
//         // Spread word animations across video duration (optional)
