...
function navbar() {
  ...
  function animateActiveHeader(index, ghostWidth) {
    ...
    // set it to its “initial” width (pre‑measured)
  ...
  sectionHeads.forEach((section, i) => {
    ...
    let tl = gsap.timeline({
      scrollTrigger: {
        ...
        //   indent: 20
        // },
        // When scrolling down into a section…
        onEnter: () => {
...
//   let sectionLinks = fanNav.querySelectorAll('.nav-menu__link[section]');
//   // console.log(sectionLinks)
...
//           // we are getting the current section num from attribute
//           // getting the previouslyactive nav-menu__link from FLYOUT
//           // if prev active, remove the active class. 
//           // add active to nav-menu__link that matches current active section

//           // used Replace
//           // const sectionNum = section.getAttribute('section');

//           //here
//           // animateSection(sectionVal, ghostItemWidth);
...
//           // Measure its current width (explicit number of pixels).
//           // let currentWidth = activeHeader.offsetWidth;
//           // // Force set its width so that GSAP can animate from that value.
//           // tl.set(activeHeader, { width: headerWidths[sliderIndex] + "px" });
...
//           // Determine the target index from the section attribute.
//           // can be done in function
//           // sliderIndex = Math.min(sectionVal - 1, navHeaders.length - 1);

//           // // used Replace
//           // // Move the container holding the nav headers. TEXT
//           // gsap.to(navSectionsInner, {
//           //   yPercent: -(sliderIndex * fraction),
//           //   duration: 0.3,
//           //   delay: .5,
...
//           // // Grab the active header.
//           // let activeHeader = navHeaders[sliderIndex];
//           // gsap.set(activeHeader, { width: headerWidths[sliderIndex] + "px" });

//           // // Measure its current width (explicit number of pixels).
//           // // let currentWidth = activeHeader.offsetWidth;
//           // // // Force set its width so that GSAP can animate from that value.
//           // // gsap.set(activeHeader, { width: currentWidth + "px" });

//           // // Animate its width to the target ghost width. WIDTH
//           // gsap.to(activeHeader, {
//           //   width: ghostItemWidth + "px",
//           //   duration: 0.3,
//           //   ease: "power4.out",
...
//     // something breaking here on scroll up
//     // Additional animations for your nav button and its wrapper.
//     // tl.to(navButton, {
//     //   width: 'auto',
//     //   duration: 0.3,
...
//           // // used Replace
//           // // Move the container holding the nav headers.
//           // gsap.to(navSectionsInner, {
//           //   yPercent: -(sliderIndex * fraction),
//           //   duration: 0.3,
//           //   delay: .5,
...
//           // // Animate its width to the target ghost width.
//           // gsap.to(activeHeader, {
//           //   width: ghostItemWidth + "px",
//           //   duration: 0.3,
//           //   ease: "power4.out",
...
//         // When scrolling upward…
//         onEnterBack: () => {
//           const sectionVal = parseInt(section.getAttribute("section"), 10);
//           sliderIndex = Math.min(sectionVal - 1, navHeaders.length - 1);
...
// shorter but not there yet, onenter and leave broken
// function navbar() {
...
//           const newActive = fanNav.querySelector(
//             `.nav-menu__link[section="${sectionNum}"]`);
...
//   // For each section, create a ScrollTrigger timeline.
//   sectionHeads.forEach((section, i) => {
//     let currentGhostNavItem = ghostNavItems[i];
//     let ghostItemWidth = currentGhostNavItem.getBoundingClientRect().width;
...
// og
// function navbar() {
//   let navButton = document.querySelector('.navbar_button');
//   let navSectionsInner = navButton.querySelector('.navbar_sections');
//   let navHeaders = navButton.querySelectorAll('.navbar_section-text');
//   let navHeaderSections = navButton.querySelectorAll('.navbar_section');
//   let navbarSectionWrapper = navButton.querySelector('.navbar_section-wrapper');
...
//           console.log('currentGhostNavItem:', currentGhostNavItem);
...
//   const fraction = 100 / navHeaders.length;
//   let sliderIndex = 0;
//   let navButtonWidth = navButton.getBoundingClientRect().width;
...
//         },
//         refreshPriority: -1, // Ensures this refreshes after others.
//         id: "Section " + i, // For marker identification.
//         markers: {
//           startColor: "lime",
//           endColor: "fuchsia",
//           fontSize: "16px",
...
//           // Use the section attribute ("section") to determine the target index.
//           const sectionVal = parseInt(section.getAttribute("section"), 10);
//           sliderIndex = Math.min(sectionVal - 1, navHeaders.length - 1);

//           // Move the container that holds all the nav headers.
//           gsap.to(navSectionsInner, {
//             yPercent: -(sliderIndex * fraction),
//             duration: 0.3,
//             ease: "power4.out"
...
//           // Use Flip to animate the active header's width change:
//           // 1. Record the current state of all nav headers.
//           let state = Flip.getState(navHeaders);
//           // 2. Update the DOM state: add "isActive" only on the one that's active.
//           navHeaders.forEach((hdr, idx) => {
//             if (idx === sliderIndex) {
//               hdr.classList.add(
//                 "isActive"); // In your CSS, .isActive sets width: fit-content, etc.
//             } else {
//               hdr.classList.remove("isActive");
//             }
//           });
...
//           // 3. Animate from the previous state to the new state.
//           Flip.from(state, {
//             duration: 5,
//             ease: "power4.out",
//             onComplete: ()=> {
...
//           // Animate with Flip for the active header.
//           let state = Flip.getState(navHeaders);
//           navHeaders.forEach((hdr, idx) => {
//             hdr.classList.toggle("isActive", idx === sliderIndex);
//           });
...
//     // Other animations for your nav button and its wrapper.
//     tl.to(navButton, {
//       width: 'auto',
//       duration: 0.3
//     }, "<");
...
//   let navbarHeight = document.querySelector('.navbar_icon').getBoundingClientRect().height;
//   gsap.set(navbarSectionWrapper, {
//     height: navbarHeight
//   })
//   gsap.set(navHeaderSections, {
//     height: navbarHeight
//   })
...
//   // We'll track which navHeader is “active” in the slider
//   let sliderIndex = 0;

//   let navButtonWidth = navButton.getBoundingClientRect().width;

//   // Initially, set only the first navHeader to isActive
//   navHeaders.forEach((header, j) => {
//     header.classList.toggle('isActive', j === sliderIndex);
//   });
...
//   sectionHeads.forEach((section, i) => {
//     let currentGhostNavItem = ghostNavItems[i];
//     // console.log(`currentGhostNavItem`)
//     // console.log(currentGhostNavItem)
//     let ghostItemWidth = currentGhostNavItem.getBoundingClientRect().width;
//     let currentIndex = i;
//     // console.log(`section`)
//     // console.log(section)
//     // need a single keeper of current section
//     let tl = gsap.timeline({
//       scrollTrigger: {
//         // refreshPriority: 1,
//         trigger: section,
...
//         // end: "bottom bottom",
//         // scrub: 1,
//         id: "Section " + i, // so the markers show a unique label
//         markers: {
//           startColor: "lime",
//           endColor: "fuchsia",
//           fontSize: "16px",
...
//           const sectionVal = parseInt(section.getAttribute("section"), 10);
//           // console.log(`sectionVal ${sectionVal}`)
//           // Because sectionVal starts at 1, shift it down by 1 so "1" => 0
//           sliderIndex = Math.min(sectionVal - 1, navHeaders.length - 1);
//           // console.log(`sliderIndex ${sliderIndex}`)
...
//         },
//         // onUpdate: self => {
//         //   // Get the section's progress (0 to 1).
//         //   // When scrolling down, once progress is >= 0.5, consider this section active.
//         //   // When scrolling up and progress falls below 0.5, switch back to previous section.
//         //   let targetIndex;
//         //   if (i === 0) {
//         //     targetIndex = 0; // first section is always index 0 when active
//         //   } else {
//         //     // If progress is less than 0.5 (i.e., scrolled up partway through the section),
//         //     // then consider the previous section active.
//         //     targetIndex = self.progress < 0.5 ? i - 1 : i;
//         //   }
//         //   // Clamp the index within bounds.
//         //   sliderIndex = Math.min(targetIndex, navHeaders.length - 1);
//         //   // Move the navbar container
//         //   gsap.to(navSectionsInner, {
//         //     yPercent: -(sliderIndex * fraction),
//         //     duration: 0.2,
//         //     ease: "power4.out"
//         //   });
//         //   // Update the active class on navbar headers:
//         //   navHeaders.forEach((hdr, idx) => {
//         //     hdr.classList.toggle("isActive", idx === sliderIndex);
//         //   });
//         // },
...
//     });
//     // console.log('sizes')
//     // console.log(navButtonWidth, ghostItemWidth)
//     // console.log(` ${navButtonWidth} + ${ghostItemWidth}px`)
//     tl.to(navButton, {
//         // width: ` ${navButtonWidth  + ghostItemWidth}px`,
//         width: 'auto',
//         backgroundColor: "purple"
//       }, "<")
//       .call(() => {
...
// function navbar() {
//   console.log('bnavbar!')
//   let navButon = document.querySelector('.navbar_button');
//   let navSectionsInner = document.querySelector('.navbar_sections');
//   let navHeaders = navSectionsInner.querySelectorAll('.navbar_section-text')
//   let navHeaderSections = document.querySelectorAll('.navbar_section')
//   let navbarSectionWrapper = document.querySelector('.navbar_section-wrapper')
//   let navbarIcon = document.querySelector('.navbar_button_lines')

//   const fraction = 100 / navHeaders.length;

//   // const shiftPercent = 
//   // const shiftPercent = -(i * fraction);
//   // console.log(`shiftPercent ${shiftPercent}`)

//   // let navButtonHeight = navButon.getBoundingClientRect().height;
//   // console.log(navButtonHeight)

//   // gsap.to(navHeaderSections, {
//   //   height: navButtonHeight + 'px',
...
//   // have to figure out new text width before starting animation to bigger width. (maybe use flip?)
//   // need current pill width
//   const sectionHeads = document.querySelectorAll("[section-head]");
//   // console.log(`sectionHeads`)
//   // console.log(sectionHeads)
//   // console.log('navHeader')
//   // console.log(navHeaders)
//   sectionHeads.forEach((section, i) => {
//     let currentIndex = i;
//     let currentSectionInnerY = 0
//     let tl = gsap.timeline({
//       scrollTrigger: {
//         trigger: section,
//         start: "top 40%",
...
//           if (currentIndex > 0) {
//             console.log('entering back current index greater than zero')
//             gsap.to(navSectionsInner, {
//               // yPercent: '+=50'
//               yPercent: `+=${fraction}`,
//             })
...
//     console.log(navHeaders[i])
//     // let curretHeader = navHeaders[i];
//     // if (i === 1) {
...
//     //   const currentHeader = navHeaders[i];
//     //   console.log(`currentHeader`)
//     //   console.log(currentHeader)
//     //   if (!currentHeader) return;
//     //   currentHeader.classList.remove('isActive');
//     // }

//     // if (i > 0) {

//     //   // console.log(`i is ${i}`)
//     //   // let currentHeader = navHeaders[i];
//     //   // currentHeader.classList.remove("isActive")

//     //   // let prevHeader = navHeaders[i - 1];
//     //   // prevHeader.classList.remove('isActive')

//     //   // let nextHeader = navHeaders[i];
//     //   // nextHeader.classList.add('isActive')
//     //   // console.log('prevHeader')
//     //   // console.log(prevHeader)

//     //   // // navHeaders[i + 1].classList.add("isActive")
...
//     prevIndex = i;
//   });
//   // navButon.addEventListener('mouseover', (e) => {
//   //   console.log('hovered')
//   // })
...
//   // Example: set heights
//   let navbarIconEl = document.querySelector(".navbar_icon");
//   if (navbarIconEl) {
//     let navbarHeight = navbarIconEl.getBoundingClientRect().height;
//     gsap.set(navbarSectionWrapper, { height: navbarHeight });
//     gsap.set(navHeaders, { height: navbarHeight });
//   }

//   // fraction used in your container shift
//   const fraction = 100 / navHeaders.length;
//   let sliderIndex = 0;

//   // Initially, only first .navbar_section-text is active
//   navHeaders.forEach((header, j) => {
//     header.classList.toggle("isActive", j === sliderIndex);
//   });

//   // 1) PIN THE NAVBAR FOR THE ENTIRE DOC
//   // We'll just pin .navbar_button from top to bottom of the body
...
//   // 2) Multi-trigger logic for each [section-head]
//   const sectionHeads = document.querySelectorAll("[section-head]");
//   sectionHeads.forEach((section, i) => {
//     let currentIndex = i;
...
//         onEnter: () => {
//           console.log("==== onEnter ====");
//           console.log(`currentIndex ${currentIndex}`, section);

//           const sectionVal = parseInt(section.getAttribute("section"), 10);
//           // Because sectionVal starts at 1, shift it down by 1 => 0-based
//           sliderIndex = Math.min(sectionVal - 1, navHeaders.length - 1);
//           console.log(`sectionVal = ${sectionVal}, sliderIndex = ${sliderIndex}`);
...
//           console.log("onLeaveBack for section", i, section);
//           const sectionVal = parseInt(section.getAttribute("section"), 10);
//           sliderIndex = Math.min(sectionVal - 1, navHeaders.length - 1);
...
//     // 3) Tweak the button tween so it unrounds:
//     //    We add height:'auto' and borderRadius:'0' (or '1rem')
//     tl.to(navButon, {
//         width: "auto",
//         height: "auto",
//         idth: "unset"
...
//           // if (currentIndex > 0) {
//           //   // sliderIndex = Math.min(sliderIndex + 1, navHeaders.length - 1);
...
//           //     header.classList.toggle('isActive', j === sliderIndex);
//           //   });
//           //   gsap.to(navSectionsInner, { yPercent: 0 });
//           // }
//           // SHIFT sliderIndex by +1, but clamp so we don't exceed last child
...
//           //   gsap.to(navSectionsInner, {
//           //     // yPercent: '+=50'
//           //     yPercent: -(newIndex * fraction),
//           //   })

//           //   // SHIFT sliderIndex down by 1, clamp at 0
...
//           //   });
//           // } else {
//           //   // If we got back to i=0 => reset sliderIndex to 0
//           //   sliderIndex = 0;
//           //   navHeaders.forEach((header, j) => {
//           //     header.classList.toggle('isActive', j === 0);
//           //   });
//           //   gsap.to(navSectionsInner, { yPercent: 0 });
//           // }
//           // console.log(progress, direction, isActive)
...
//   console.log('bnavbar!')
//   let navButon = document.querySelector('.navbar_button');
//   let navSectionsInner = navButon.querySelector('.navbar_sections');
//   let navHeaders = navButon.querySelectorAll('.navbar_section-text')
//   let navHeaderSections = navButon.querySelectorAll('.navbar_section')
//   let navbarSectionWrapper = navButon.querySelector('.navbar_section-wrapper')
//   let navbarIcon = navButon.querySelector('.navbar_button_lines')
...
// 11:24am here here here og BEST
// function navbar() {
//   console.log('bnavbar!')
//   let navButon = document.querySelector('.navbar_button');
//   let navSectionsInner = document.querySelector('.navbar_sections');
...
// navbar()
// function navbar() {
//   let navBarButtonSectionWrap = document.querySelector('.navbar_button-section')
//   let navButton = document.querySelector('.navbar_button');
//   let navSectionsInner = document.querySelector('.navbar_sections-inner');
//   let navHeaders = document.querySelectorAll('.navbar_section-text')
//   let navHeaderSections = document.querySelectorAll('.navbar_section')
//   let navbarSectionWrapper = document.querySelector('.navbar_sections-wrapper')
//   let navbarIcon = document.querySelector('.navbar_button_lines')

//   let navButtonHeight = navButton.getBoundingClientRect().height;
//   console.log(navButtonHeight)
...
//     console.log('currentHeader')
//     console.log(navHeaders[i])
//     console.log('currentHeaderSection')
//     let currentHeaderSection = navHeaderSections[i];
//     console.log(currentHeaderSection)

//     let nextWidth = currentHeaderSection.querySelector('.navbar_section-text')
//       .getBoundingClientRect().width;
//     console.log(`next w is ${nextWidth}`)
//     let currentHeader = navHeaders[i];
//     if (i === 1) {
//       tl.to(navButton, {
//         width: () => navButton.offsetWidth + nextWidth + 'px'
//       })
...
//   // Just an example of sizing all navHeaders
//   let navButtonHeight = navButton.getBoundingClientRect().height;
//   gsap.to(navHeaderSections, {
//     height: navButtonHeight + 'px',
//     border: "1px solid white"
...
//     console.log("Section index:", i, section);

//     // We want Tagline for i=1 → .navbar_section[0],
//     // so let's do i - 1
//     let currentHeaderSection = navHeaderSections[i - 1];
//     if (!currentHeaderSection) return;

//     // We'll measure the text's width so we can grow .navbar_button
//     let nextWidth = currentHeaderSection
//       .querySelector('.navbar_section-text')
//       .getBoundingClientRect().width;
//     console.log(`next w is ${nextWidth}`);

//     // Example: if i === 1 => handle Tagline
//     if (i === 1) {
//       // Step A) define a .add() step so it runs inside the timeline
//       tl.add(() => {
//         // 1) capture Flip state for that .navbar_section
//         const state = Flip.getState(currentHeaderSection);

//         // 2) move it to .navbar_button-section as the first child
//         navBarButtonSectionWrap.prepend(currentHeaderSection);
...
//       // Step B) also animate the navButton width
//       tl.to(navButton, {
//         width: () => {
//           const baseWidth = navButton.getBoundingClientRect().width;
//           return baseWidth + nextWidth + "px";
//         },
//         duration: 0.4
//       });
//     }
...
// navbar();
// function navbar() {
//   const navButon = document.querySelector('.navbar_button');
//   const sectionHeads = document.querySelectorAll("[section-head]");

//   sectionHeads.forEach((section, index) => {
//     let tl = gsap.timeline({
//       scrollTrigger: {
//         trigger: section,
//         start: "top 40%",
...
//     tl.add(() => {
//       // capture initial Flip state
//       const state = Flip.getState(
//         navButon, { props: "backgroundColor,color,width,height" });

//       // Apply a class for this section
//       navButon.classList.remove("showTagline", "showBrand", "showWhatever");
//       if (index === 1) navButon.classList.add("showTagline");
//       if (index === 2) navButon.classList.add("showBrand");
//       if (index === 3) navButon.classList.add("showWhatever");

//       // flip from the previous state
//       Flip.from(state, {
//         duration: 0.6,
//         ease: "power1.inOut",
//         absolute: true
...
// function navbar() {
//   const navButon = document.querySelector('.navbar_button');
//   const navHeaders = document.querySelectorAll('.navbar_section-text');
//   const navbarSectionWrapper = document.querySelector('.navbar_sections-wrapper');
//   const navbarIcon = document.querySelector('.navbar_button_lines');
//   const sectionHeads = document.querySelectorAll("[section-head]");

//   // Only setting y, not opacity — this fixes the text not showing
//   gsap.set(navHeaders, { y: 50 });
...
//         markers: true,
//         onEnter: () => switchHeader(i),
//         onLeaveBack: () => switchHeader(i - 1 >= 0 ? i - 1 : 0)
//       }
//     });
//   });

//   function switchHeader(i) {

//     if (i === prevIndex) return;

//     const tl = gsap.timeline();
//     const headerIndex = i - 1;
//     const newHeader = navHeaders[headerIndex];
//     const oldHeader = navHeaders[prevIndex - 1];

//     // 🔁 COLLAPSE TO CIRCLE
//     if (i === 0) {
//       navHeaders.forEach(header => header.classList.remove("isActive"));
...
//     // 🔀 OUTGOING HEADER
//     if (prevIndex > 0 && oldHeader) {
//       oldHeader.classList.remove("isActive");
//       tl.to(oldHeader, {
//         y: -50,
//         duration: 0.3
//       }, 0);
...
//       // 🔸 Animate in the new header text
//       tl.to(newHeader, {
//         y: 0,
//         paddingRight: "1rem",
//         duration: 1 // match original timing
//       }, "<");
...
//       // 🔸 Delay hiding the icon until text is fully in
//       tl.to(navbarIcon, {
//         width: 0,
//         overflow: "hidden",
//       }, "<")
...
//     // if (i === 2) {
//     //   console.log('brand ident!')
//     //   const taglineHeader = navHeaders[0]; // Tagline
//     //   const brandHeader = navHeaders[1]; // Brand Identity
...
//     //   // ✅ Add active to Brand Identity
//     //   brandHeader.classList.add("isActive");
...
//     //   // ✅ Reset tagline styles just in case
//     //   tl.set(taglineHeader, { clearProps: "all" });

//     //   // ✅ Ensure navbar & wrapper are expanded
//     //   tl.to(navButon, {
//     //     width: 'auto',
//     //   });
...
//     //   // ✅ Animate brand identity in
//     //   tl.fromTo(brandHeader, {
//     //     y: 50
//     //   }, {
//     //     y: 0,
//     //     paddingRight: "1rem",
...
//       const taglineHeader = navHeaders[0]; // Tagline
//       const brandHeader = navHeaders[1]; // Brand Identity
//       const addPadding = prevIndex === 0;

//       // Clear all active states
//       navHeaders.forEach(header => header.classList.remove("isActive"));
//       brandHeader.classList.add("isActive");

//       // Force reflow after .isActive so we can measure true width
//       const targetButtonWidth = navButon.getBoundingClientRect().width;

//       // Set current width to avoid jump
//       tl.set(navButon, { width: navButon.offsetWidth });
...
//       // Clear tagline styles after
//       tl.set(taglineHeader, { clearProps: "all" });

//       // Animate nav button to the new width
//       tl.to(navButon, {
//         width: targetButtonWidth,
//         duration: 0.3
//       }, 0);

//       // Animate in brand identity
//       tl.fromTo(brandHeader, {
//         y: 50
//       }, {
//         y: 0,
...
function revealText(textRevealWrapper, isPageHeader = false) {
  ...
  textRevealContents.forEach((text, i) => {
    ...
    if (fastSVGElement && hasSVGAnimation) {
      ...
      // ↑ "+=0.5" = wait 0.5s *after* previous step
...
//       }, "-=0.2"); // 🔹 Add delay to prevent overlapping transitions
//     }
...
//         revealMasterTl.play(); // ✅ Resume after animation finishes
//       }
//     })
//     let firstContents = textRevealContents[0];
//     let text = firstContents.querySelectorAll('.line-item__inner');
...
function kineticText() {
  ...
  kineticSections.forEach((section) => {
    ...
    gsap.set(words, {
      // Make sure they’re invisible and moved downward
      opacity: 0,
      y: 30,
...
//         scroller: "body" // ✅ Ensure Lenis or default scroll works
//       }
//     })
...
html {
  ...
  .navbar_sections .navbar_section:nth-of-type(1) .navbar_section-text {
    padding: 0;
  }
