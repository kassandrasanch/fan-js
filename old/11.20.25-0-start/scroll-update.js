...
function setupPageSlider(wrapper) {
    ...
    const tops = slides.map(slide => ScrollTrigger.create({
        trigger: slide,
        start: "top top",
        // markers: true,
    }))
    ...
    const tl = gsap.timeline({
        scrollTrigger: {
            ...
            id: "page-slider",
            // snap: (rawProgress) => {
            //     // Build array of label progress values once
            //     if (!tl._labelPositions) {
            //         tl._labelPositions = slides
            //             .map((_, i) => {
            //                 const t = tl.labels[`slide-${i}`];
            ...
            //     const labels = tl._labelPositions;

            //     // Find which segment we're in: [labels[i], labels[i+1]]
            //     let segIndex = 0;
            ...
            //     // Only snap within, say, 20% of the segment edges
            //     const EDGE_PORTION = 0.2; // 20% of segment
            //     const leftSnapZoneEnd = segStart + segLen * EDGE_PORTION;
            //     const rightSnapZoneStart = segEnd - segLen * EDGE_PORTION;

            //     // If we're in the middle 60% of the segment -> no snap at all
            //     if (rawProgress > leftSnapZoneEnd && rawProgress < rightSnapZoneStart) {
            //         return rawProgress;
            //     }

            //     // Otherwise, snap to the nearest edge of this segment
