...
function setupBigSlider(wrapper) {
  ...
  //    • panel[0] & img[0] stay in place (xPercent:0)
  //    • all others start offscreen right (panels) / left (imgs)
  panels.forEach((panel, i) => {
    gsap.set(panel, { xPercent: i === 0 ? 0 : 100 });
    gsap.set(imgs[i], { xPercent: i === 0 ? 0 : -100 });
  });
  ...
  panels.forEach((panel, i) => {
    ...
    tl.to(prev, { xPercent: -100, duration: 1, ease: 'power4.inOut' }, '<')
      .to(prevImg, { xPercent: 100, duration: 1, ease: 'power4.inOut' }, '<')
      .fromTo(panel, { xPercent: 100 }, { xPercent: 0, duration: 1, ease: 'power4.inOut' }, '<')
      .fromTo(curImg, { xPercent: -100 }, { xPercent: 0, duration: 1, ease: 'power4.inOut' },
        '<');
  });
}
