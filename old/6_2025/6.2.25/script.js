  ...
  mutations.forEach(mutation => {
    if (
      ...
      if (isOpen) {
        ...
        // 0439 here its weirdly broken. maybe i should just add a copy of the navheader here. tricky bc im trying to use the webflow one. 
        document.querySelector('.n_nav__inner').style.backgroundColor = '#e10600'
        gsap.to('.n_nav__inner', {
        ...
        // Recalculate height on open, in case header changed!
        const navHeaderHeight = navHeader.getBoundingClientRect().height;
        navMenuPrimary.style.marginTop = navHeaderHeight + 'px';
        // navMenu.style.height = `calc(100vh - ${navHeaderHeight}px)`;
        // If you want max-height for animation, use maxHeight instead.
        // navMenu.style.maxHeight = `calc(100vh - ${navHeaderHeight}px)`;
      } else {
        document.querySelector('.n_nav__inner').style.backgroundColor = 'transparent'
        ...
        // navMenu.style.height = ""; // Clear inline height
        // gsap.to(navBrand, {
        //   opacity: 0
        // })

      }
      console.log('aria-expanded changed:', isOpen ? 'OPEN' : 'CLOSED');
    }
  });
});
// Start observing
observer.observe(navToggle, {
  ...
  attributeFilter: ['aria-expanded'], // Only this attribute
});
...
function createWordTimeline(words, block) {
  ...
  if (fastSVGElement) {
    console.log(`fastSVGElement FOUND FOUND FOUND`)
  } else {
    console.log(`fastSVGElement NOT found`)
  }
...
function revealText(wrapper, isPageHeader = false) {
  ...
  blocks.forEach((block, i) => {
    ...
    const wordAnim = createWordTimeline(words, block);
    tl.add(wordAnim);
...
// other: function revealText(textRevealWrapper, isPageHeader = false) {
...
//     let fansVideo = pageHeader.querySelector('.fans-background-video');
...
//         ease: "power2.out",
//         onComplete: () => {
//           // pageHeader.querySelector('.fans-background-video').style.zIndex = '-10';
//           // pageHeader.querySelector('.fans-background-video').style.pointerEvents = 'none';
//         }
//       });
//       preFade.to(fansVideo, {
//         zIndex: '-10',
//         pointerEvents: "none"
//       })
//       revealMasterTl.add(preFade, 0); // add it at the beginning of master
//     }
...
//   if (isPageHeader && logoVideo) {
//     const lastText = textRevealContents[textRevealContents.length - 1];
//     const lastWords = lastText.querySelectorAll('.line-item__inner');

//     revealMasterTl.to(lastWords, {
//       yPercent: 100,
//       opacity: 0,
//       duration: 0.8,
//       ease: 'power3.inOut',
...
//     });
//     // hold for a scroll 
//     revealMasterTl.to({}, { duration: 1.5 });
//   }
// }

// function splitTextIntoWords(blockEl) {
...
//   ScrollTrigger.addEventListener("refreshInit", measureWidths);

//   function onWake() {
//     measureWidths();
//     ScrollTrigger.refresh(true);
//     gsap.set(navSectionsInner, {
//       yPercent: -(sliderIndex * fraction)
//     });
//     animateActiveHeader(sliderIndex, ghostWidths[sliderIndex]);
//   }
...
//   let textRevealContents = textRevealWrapper.querySelectorAll('.text-reveal-content')
//   let fanSansHidden = false;
//   // let fanSansVideo = document.querySelector('.page-header .fans-background-video');
