...
//   // 3.1) If this block has data‐animate‐special="fast-svg", grab that .fast-svg element:
//   let fastSVGElement = null;
//   if (block.getAttribute('data-animate-special') === 'fast-svg') {
//     const section = block.closest('.n_text-reveal-section');
//     fastSVGElement = section ? section.querySelector('.fast-svg') : null;
...
//   // words are already offscreen
//   // prepareWords(words);

//   // 3.2) Create a mini timeline that “slides words in”:
//   const tl = animateWordsIn(words, color);

//   // let anton = null;
//   // if (block.getAttribute('data-animate-special') === "anton") {
//   //   anton = block.querySelector('.n_text-reveal__special--anton');
//   //   console.log('HAS ANTON')
...
//     // if no logos, hold words and then animate words out
//   } else {

//     tl.add(animateWordsOut(words));
//   }

//   // 3.4) If this is NOT the last heading in this block, STOP here and return that “slide-in” timeline:
//   if (!isLastHeading) {
//     return tl;
//   }
...
function revealText(wrapper) {
  ...
  blocks.forEach((block, i) => {
    ...
    if (block.dataset.animateSpecial === 'anton') {
      ...
      //   let words = splitTextIntoWords(h1);

      //   prepareWords(words)

      //   tl.add(animateWordsIn(words, endColor))
      //   tl.add(animateWordsOut(words), "+=0.5");
