...
function revealText(wrapper, isPageHeader = false) {
  ...
  blocks.forEach((block, i) => {
    ...
    headings.forEach((h1, headingIndex) => {
      // a) Split the text into individual <span> words:
      const words = splitTextIntoWords(h1);

      // b) Start every word “hidden below” and red by default:
      gsap.set(words, { yPercent: 100, opacity: 0, color: '#E10600' });

      // c) Figure out if this is the very last heading of the very last block:
      const isLastBlock = (i === blocks.length - 1);

      const isLastHeading = headingIndex === headings.length - 1;
      // const endColor = '#000'; // or whatever your “after‐reveal” color should be
      // const makeLast = isLastBlock && isLastHeading;

      // d) Build a mini‐timeline that tweens each “word” in/out:
      const wordAnim = createWordTimeline(words, endColor, block, isLastBlock,
        isLastHeading);

      // e) Add that mini‐timeline into our main tl:
      tl.add(wordAnim);
    });
    // // testing
    // const hasLogos = block.classList.contains('n_logo-reveal__block');
    ...
    // const wordAnim = createWordTimeline(words, endColor, block, isLast);
    // tl.add(wordAnim);
