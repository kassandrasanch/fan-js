...
//   // const posterUrl = 'https://cdn.prod.website-files.com/67b4c8583d604cb6c2fc9a62%2F6846f698ebd9174648185226_2025_06_06_EASCI_Fanatics_Ripple_01-poster-00001.jpg'
...
//   // 1) Build a master timeline whose axis = video time
//   const masterTl = gsap.timeline({ defaults: { ease: "none" } })
//     .to(video, { currentTime: video.duration, duration: video.duration }, 0);

//   blocks.forEach((block, i) => {
//     let at = parseFloat(block.getAttribute('data-animate-special'));
...
//   video.addEventListener('timeupdate', () => masterTl.seek(video.currentTime, false));
...
function initVideoTextScrub(wrapper) {
  ...
  console.log('video-wrapper')
  console.log(wrapper)
  // 1) Pull data-attrs from the Webflow wrapper
  const bg = wrapper.querySelector('.ripple-gradient');
  const posterUrl = bg.getAttribute('data-poster-url');
  const urls = bg.getAttribute('data-video-urls')
    .split(',')
    .map(s => s.trim());

  // 2) Remove the old Webflow video (so it never auto-plays)
  bg.innerHTML = '';

  // 3) Build our own <video>
  ...
  video.addEventListener('loadedmetadata', () => {
    ...
    // 7) Build GSAP timeline tied to video time
    const blocks = gsap.utils.toArray(
      wrapper.querySelectorAll('.n_text-reveal__block')
    );
...
//   // remove Webflow’s autoplaying video
//   bg.innerHTML = '';

//   // build our controlled <video>
//   const video = document.createElement('video');
//   video.preload = 'metadata';
//   video.playsInline = true;
//   video.muted = true;
...
//     // grab & split off the first block
//     const allBlocks = gsap.utils.toArray(wrapper.querySelectorAll('.n_text-reveal__block'));
//     const firstBlock = allBlocks.shift();

//     // build + prep the first heading’s words
//     const firstH1 = firstBlock.querySelector('.n_text-reveal__heading');
//     const firstWords = splitTextIntoWords(firstH1);
//     prepareWords(firstWords);

//     // create its mini-timeline
//     const firstAnim = createWordTimeline(
//       Array.from(firstWords),
//       "#ffffff",
//       firstBlock,
//       false, // not last block
//       true // it is the only heading in this mini-timeline
//     );

//     // now build the master timeline, tied to scroll
//     const masterTl = gsap.timeline({
//       paused: true,
//       defaults: { ease: 'none' },
//       scrollTrigger: {
//         trigger: wrapper,
//         start: 'top top',
//         end: () => `+=${allBlocks.length * 1400 + firstAnim.duration() * 1000}`,
//         pin: true,
//         scrub: 0,
//       }
//     });

//     // 1) add the first heading animation at t=0
//     masterTl.add(firstAnim, 0);

//     // 2) only **after** the firstAnim finishes, start scrubbing the video
//     const offset = firstAnim.duration(); // in seconds
//     masterTl.to(video, {
//       currentTime: video.duration,
//       duration: video.duration
//     }, offset);

//     // 3) schedule the rest of the blocks, each at 
//     //    (offset + its own data-start or equal spacing)
//     const perBlock = video.duration / allBlocks.length;
//     allBlocks.forEach((block, i) => {
//       const startTime = offset + (parseFloat(block.dataset.start) || perBlock * i);

//       // split & prep each heading
//       block.querySelectorAll('.n_text-reveal__heading').forEach(h1 => {
//         const words = splitTextIntoWords(h1);
//         prepareWords(words);
//         const isLastBlock = i === allBlocks.length - 1;
//         const isLastHeading = true; // assuming one heading per block

//         const wordAnim = createWordTimeline(words, "#ffffff", block, isLastBlock,
//           isLastHeading);
//         masterTl.add(wordAnim, startTime);
//       });
//     });
...
function revealText(wrapper) {
  ...
  if (isRippleGradient) {
    console.log('is ripple gradient!')
...
function initScrollingSlider() {
  ...
  // 3) header reveal (outside of the scrub timeline)
