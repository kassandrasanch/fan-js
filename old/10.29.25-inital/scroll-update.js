...
// // ---------- CONFIG ----------
// const SECTION_SEL = ".observer-section";
// const PANEL_SEL   = ".section-banner, .n_section-header, .section-split";
// // ----------------------------

// let allowScroll = true;
// const observerSections = gsap.utils.toArray(SECTION_SEL);
// let currentSection = 0;

// const sectionIndices = new Array(observerSections.length).fill(0);
// const sectionsPanels = [];
// const sectionsLimits = [];
// const sectionInited  = new Array(observerSections.length).fill(false);

// // throttle via delayedCall (like the demo)
// const scrollTimeout = gsap.delayedCall(0.6, () => (allowScroll = true)).pause();
...
//     intentObserver.disable(); // resume native scroll out of this section
//     return;
//   }
...
//     ? sectionsPanels[currentSection][sectionIndices[currentSection]]  // move current out
//     : sectionsPanels[currentSection][index];                          // bring prev back
...
//   // update memory + persist on the section element
//   sectionIndices[currentSection] = index;
//   observerSections[currentSection].dataset.panelIndex = String(index);
// };
...
// // Helper: normalize transforms once to match a stored index (no visual jump later)
// function initSectionTransforms(i) {
//   if (sectionInited[i]) return;
//   sectionInited[i] = true;
//   const idx = sectionIndices[i] || 0;
//   const panels = sectionsPanels[i] || [];
//   // panels before idx are at -100, current & after at 0
//   gsap.set(panels, { yPercent: (j) => (j < idx ? -100 : 0) });
// }

// // Build each section like the demo
// observerSections.forEach((section, i) => {
//   const panels = gsap.utils.toArray(PANEL_SEL, section);
//   if (!panels.length) return;

//   sectionsPanels[i] = panels;
//   sectionsLimits[i] = panels.length;

//   // If page loaded after user had scrolled before (SPA/hard reload),
//   // read any persisted index (else 0)
//   const savedIdx = parseInt(section.dataset.panelIndex ?? "0", 10) || 0;
//   sectionIndices[i] = Math.min(Math.max(savedIdx, 0), panels.length - 1);

//   // stack highest zIndex on first panel so the next one is revealed underneath
...
//     end: "+=200",
//     pin: true,
//     markers: true, // turn off when done
//     // IMPORTANT: no `scrub` here

//     onEnter(self) {
//       if (intentObserver.isEnabled) return;
//       self.scroll(self.start + 1); // hold at the top of pin
//       intentObserver.enable();
//       currentSection = i;

//       // sync memory from persisted attribute (no movement)
//       const stored = parseInt(section.dataset.panelIndex ?? String(sectionIndices[i]), 10) || 0;
//       sectionIndices[i] = Math.min(Math.max(stored, 0), panels.length - 1);

//       // on first ever entry, normalize transforms to match current index (no jump later)
//       initSectionTransforms(i);
//     },

//     onEnterBack(self) {
//       if (intentObserver.isEnabled) return;
//       self.scroll(self.end - 1); // hold at the bottom of pin
//       intentObserver.enable();
//       currentSection = i;
...
// // throttle via delayedCall (like the demo)
// const scrollTimeout = gsap.delayedCall(0.6, () => (allowScroll = true)).pause(); // 0.6s feels snappier than 1s
...
//   allowScroll = false;
//   scrollTimeout.restart(true); // re-arm the cooldown
...
//   preventDefault: true,
//   onEnable(self) {
//     allowScroll = false;                // match the demo
//     scrollTimeout.restart(true);
//     const savedScroll = self.scrollY(); // freeze native scroll inside pin
//     self._restoreScroll = () => self.scrollY(savedScroll);
//     document.addEventListener("scroll", self._restoreScroll, { passive: false });
//   },
//   onDisable(self) {
...
//     end: "+=200",
//     pin: true,
//     markers: true, // set true if you want to debug
//     scrub: true,
//     onEnter(self) {
//       if (intentObserver.isEnabled) return;
//       self.scroll(self.start + 1); // hold at the top of pin
...
// try 7 Left off here. its not working. added scrolltimeout back in but still jacked
// issues
// nav zindex issue
...
//     // --- Boundary handling (BEFORE touching arrays)
//     // top-of-page edge: first section, trying to go above first panel
//     if (!isScrollingDown && index < 0) {
//         if (currentSection === 0) {
//             dbg("TOP EDGE: section 0 & index<0 -> ignore, keep observer enabled");
//             allowScroll = true;
//             updateHUD();
//             return;
//         }
//         dbg("LEAVING SECTION UP: disabling observer & nudging above pin");
//         intentObserver.disable();
//         allowScroll = true;
//         const st = sectionTriggers[currentSection];
//         if (st) {
//             dbg("nudge scroll -> start-1", st.start - 1);
...
//     // bottom-of-page edge: last section, trying to go past last panel
//     const isLastSection = currentSection === panelSections.length - 1;
//     if (isScrollingDown && index > last) {
//         if (isLastSection) {
//             dbg("BOTTOM EDGE: last section & index>last -> ignore, keep observer enabled");
//             allowScroll = true;
//             updateHUD();
//             return;
//         }
//         dbg("LEAVING SECTION DOWN: disabling observer & nudging past pin");
//         intentObserver.disable();
//         allowScroll = true;
//         const st = sectionTriggers[currentSection];
//         if (st) {
//             dbg("nudge scroll -> end+1", st.end + 1);
//             st.scroll(st.end + 1);
//         }
//         updateHUD();
//         return;
...
//     const fromIndex = sectionIndices[currentSection];
//     const target = isScrollingDown
//         ? sectionsPanels[currentSection][fromIndex]
//         : sectionsPanels[currentSection][index];

//     dbg("Animating", { target, yTo: isScrollingDown ? -100 : 0 });
...
// console.log(panelSections);
// // try 6 CLOSEST WORKS WHEN SCROLLING SLOWWWWLLLYYYY
// // issues
// // nav zindex issue
// // scrollbar not showing full height issue
// // text sections
...
// console.log(panelSections);
// // try 5.5ag does notloop || pos absolute panels || with debug and panel lock
// issues
// nav zindex issue
// scrollbar not showing full height issue
// text sections
...
//     // onDown: () => allowScroll && goToPanel(sectionIndices[currentSection] + 1, true),
//       onDown: () => {
//     if (gestureLocked || !allowScroll) return;
//     gestureLocked = true;
...
// // // try 5.5af does notloop || pos absolute panels
// // issues
// // nav zindex issue
// // scrollbar not showing full height issue
// // text sections
...
//     // console.log(sectionsPanels[currentSection][sectionIndices[currentSection]]);
//     // console.log(`target 2`);
//     // console.log(sectionsPanels[currentSection][index]);
//     const target = isScrollingDown
...
// // try 5.5ae does notloop || pos absolute panels
// issues
// nav zindex issue
// scrollbar not showing full height issue
// text sections
...
// let currentIndex = -1;
// let animating;
// const last = Math.max(0, scrollPanels.length - 1);
// // use observer in text reveal anis?
...
// //     if (pageY(scrollPanels[i]) <= y) idx = i;
// //   return idx;
// // }

// // let animating = false;
// // const last = Math.max(0, scrollPanels.length - 1);
// // const clampIndex = (i) => Math.min(last, Math.max(0, i));
// // let currentIndex = getCurrentIndex();

// function goToSectionBody(index, direction) {
//     animating = true;
//     if (index < 0 || index > last || index === currentIndex) return;
//     let fromTop = direction === -1;
//     let dFactor = fromTop ? -1 : 1; // direction
//     tl = gsap.timeline({
...
//     if(currentIndex >= 0) {
//         gsap.set(scrollPanels[currentIndex], {
//             zIndex: 0
//         })
//     }
//     gsap.set(scrollPanels[index], { autoAlpha: 1, zIndex: 1 });
...
//   preventDefault: true,
//   tolerance: 12,
//   onDown: ()=> !animating && goToSectionBody(currentIndex + 1, 1),
//   onUp: ()=> !animating && goToSectionBody(currentIndex - 1, -1),
// //   onUp: () => {
// //     if (!animating && currentIndex > 0) {
// //       goToSectionBody(currentIndex - 1, -1);
// //     }
// //   },
// //   onDown: () => {
// //     if (!animating && currentIndex < last) {
// //       goToSectionBody(currentIndex + 1, 1);
// //     }
// //   }
// });

// // try 5.5ad does notloop || pos absolute panels
// issues
// nav zindex issue
// scrollbar not showing full height issue
// text sections
// what abt panels that are taller/shorter than viewport?
// ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// --- helpers (added clamp to max scroll + naturalTop remains) ---
// ================== ABSOLUTE SLIDES + EXISTING TWEEN/OBSERVER ==================

// 0) Grab wrapper + panels (your classes)
// const wrapper = document.querySelector('.main-wrapper');

// // -------------------- helpers you already use --------------------
// const getScroll = () => window.scrollY || document.documentElement.scrollTop || 0;

// // if absolute layout set __absTop, use it; else fallback
// const pageY = (el) => (typeof el.__absTop === 'number')
//   ? el.__absTop
//   : (el.getBoundingClientRect().top + getScroll());

// function naturalTop(el) {
//   return (typeof el.__absTop === 'number') ? el.__absTop : el.offsetTop;
// }

// // real max scroll against wrapper height (we set it in layoutAbsolute)
// function docMaxScroll() {
//   const total = wrapper?.offsetHeight || document.body.scrollHeight;
...
//   let idx = 0;
//   for (let i = 0; i < scrollPanels.length; i++) {
//     if (pageY(scrollPanels[i]) <= y) idx = i; else break;
//   }
//   return idx;
// }
...
// // -------------------- absolute layout (gives real scrollbar) --------------------
...
//     const naturalH = Math.max(p.scrollHeight, r.height);
//     const step = Math.max(naturalH, vh); // one “deck step” per slide

//     // stack in the wrapper
...
//   // real scrollbar: wrapper height is sum of steps
//   wrapper.style.position = 'relative';
//   wrapper.style.height = acc + 'px';

//   currentIndex = getCurrentIndex();
// }

// // -------------------- pin/unpin (stay absolute; lift z-index) --------------------
// function pinPanel(panel) {
//   gsap.set(panel, {
//     position: 'absolute',   // important: DO NOT switch to fixed
//     left: 0,
//     width: '100%',
//     zIndex: 10001,
...
//   gsap.set(panel, {
//     clearProps: 'willChange,zIndex,backfaceVisibility,transform,y' // keep position/top
//   });
// }

// // -------------------- your transition (bare visual slide) --------------------
// function goToSectionBody(index, direction) {
//   if (animating) return;
//   if (index < 0 || index > last || index === currentIndex) return;
...
//   const prev = currentIndex >= 0 ? scrollPanels[currentIndex] : null;
//   const next = scrollPanels[index];

//   if (prev) pinPanel(prev);
//   pinPanel(next);

//   // Use PX distances based on each slide's step (not yPercent)
//   const nextStep = next.__step || Math.max(next.getBoundingClientRect().height, window.innerHeight);
//   const prevStep = prev ? (prev.__step || Math.max(prev.getBoundingClientRect().height, window.innerHeight)) : 0;

//   gsap.set(next, { autoAlpha: 1, zIndex: 10001, y: nextStep * dFactor });
//   if (prev) gsap.set(prev, { zIndex: 10000, y: 0 });

//   const tl = gsap.timeline({
//     defaults: { duration: 0.9, ease: 'power2.inOut' },
//     onComplete: () => {
//       // snap the document scroll so the scrollbar matches the landed slide
//       const targetTop = Math.min(docMaxScroll(), Math.round(naturalTop(next)));
//       window.scrollTo(0, targetTop);

//       requestAnimationFrame(() => {
//         if (prev) { gsap.set(prev, { clearProps: 'y' }); unpinPanel(prev); }
//         gsap.set(next, { clearProps: 'y,willChange' }); // keep position/top
//         unpinPanel(next);
...
//   // freeze native scroll while tween runs (no drift)
//   const freezeY = getScroll();
//   tl.eventCallback('onUpdate', () => window.scrollTo(0, freezeY));

//   // tween in PX, not %
//   if (prev) tl.to(prev, { y: -prevStep * dFactor }, 0);
//   tl.to(next, { y: 0 }, 0);
// }

// // -------------------- Observer: 1 gesture => 1 slide --------------------
// Observer.create({
//   target: window,
//   type: 'wheel,touch,pointer',
//   preventDefault: true,
//   tolerance: 12,
//   onUp:   () => { if (!animating && currentIndex > 0)      goToSectionBody(currentIndex - 1, -1); },
//   onDown: () => { if (!animating && currentIndex < last)   goToSectionBody(currentIndex + 1,  1); }
// });

// // -------------------- init --------------------
// layoutAbsolute();
// currentIndex = getCurrentIndex();
// window.scrollTo(0, scrollPanels[currentIndex]?.__absTop ?? 0);

// // keep deck math fresh on resize (and stay at same logical slide)
// window.addEventListener('resize', () => {
//   if (animating) return;
//   const idx = currentIndex;
//   layoutAbsolute();
//   window.scrollTo(0, scrollPanels[idx]?.__absTop ?? 0);
//   currentIndex = idx;
// }, { passive: true });

// // try 5.5ac does notloop || pos absolute panels
// issues
// nav zindex issue
// scrollbar not showing full height issue
// text sections
...
// // 1) Helpers (tweaked to respect absolute layout)
// const getScroll = () => window.scrollY || document.documentElement.scrollTop || 0;

// // When absolute layout is active, use cached absolute top (__absTop); otherwise fallback
// const pageY = (el) => (typeof el.__absTop === 'number')
//   ? el.__absTop
//   : (el.getBoundingClientRect().top + getScroll());
...
// }
// function naturalTop(el) {
//   // Points to the absolute deck position if we laid out; else fallback
//   return (typeof el.__absTop === 'number')
//     ? el.__absTop
//     : (el.offsetTop - getMarginTop(el));
// }
...
// //     const naturalH = Math.max(p.scrollHeight, rect.height);
// //     const step = Math.max(naturalH, vh); // one deck step per slide

// //     // place absolutely in the deck
// //     p.style.position = 'absolute';
// //     p.style.left = '0';
// //     p.style.width = '100%';
// //     p.style.top = acc + 'px';
...
// //   // make the wrapper tall so the native scrollbar covers the full journey
// //   wrapper.style.height = acc + 'px';

// //   // sync index and (optionally) clamp scroll to nearest slide
// //   currentIndex = getCurrentIndex();
// // }

// // // 3) Pin/Unpin (stay absolute; just lift z-index + layer hint)
// // function pinPanel(panel) {
// //   gsap.set(panel, {
// //     position: "absolute",
// //     left: 0,
...
// function unpinPanel(panel) {
//   gsap.set(panel, {
//     clearProps: "willChange,zIndex,backfaceVisibility,transform,y" // <-- includes y
//   });
// }

// // 4) Transition: keep your GSAP timeline semantics (prev out, next in)
// function goToSectionBody(index, direction) {
//   if (animating) return;
//   if (index < 0 || index > last || index === currentIndex) return;
...
//   const fromTop = direction === -1;
//   const dFactor = fromTop ? -1 : 1;
//   const sections = scrollPanels;

//   const prev = currentIndex >= 0 ? sections[currentIndex] : null;
//   const next = sections[index];

//   if (prev) pinPanel(prev);
//   pinPanel(next);

//   // --- INIT POSITIONS (use px, not %) ---
//   const nextStep = next.__step || Math.max(next.getBoundingClientRect().height, window.innerHeight);
//   const prevStep = prev ? (prev.__step || Math.max(prev.getBoundingClientRect().height, window.innerHeight)) : 0;
...
// // 5) Observer: one gesture => one slide (we’ll add tall-slide proximity later)
// Observer.create({
//   target: window,
//   type: "wheel,touch,pointer",
//   preventDefault: true,
...
// // 6) Init + responsive relayout
// function initAbsoluteDeck() {
//   layoutAbsolute();
//   // Optionally snap to the nearest on load (comment out if you want to keep current scroll)
...
// window.addEventListener('resize', () => {
//   if (!animating) {
//     const yBefore = getScroll();
//     layoutAbsolute();
//     // keep user at the same logical slide after relayout
//     const idx = getCurrentIndex();
//     window.scrollTo(0, scrollPanels[idx]?.__absTop ?? yBefore);
//     currentIndex = idx;
//   }
// }, { passive: true });
// // try 5.5ab does notloop || CLOSEST
// issues
// nav zindex issue
// scrollbar not showing full height issue
// text sections
...
// --- helpers (added clamp to max scroll + naturalTop remains) ---
// const getScroll = () => window.scrollY || document.documentElement.scrollTop || 0;
// const pageY = (el) => el.getBoundingClientRect().top + getScroll();

// function getMarginTop(el) {
//   return parseFloat(getComputedStyle(el).marginTop) || 0;
// }
// function naturalTop(el) {
//   return el.offsetTop - getMarginTop(el);
// }
// // max scroll guard prevents overshoot on the last panel (fixes blink at end)
// function docMaxScroll() {
...
// // --- pin/unpin (keep transform; avoid layer drop) ---
// function pinPanel(panel) {
//   const rect = panel.getBoundingClientRect();

//   // placeholder keeps layout height while fixed
...
//   // fixed + high z + own layer
//   gsap.set(panel, {
//     // this worked is pos is fixed in css too.
//     position: "fixed",
//     top: 0,
//     left: 0,
//     width: "100%",
...
//     force3D: true,
//     backfaceVisibility: "hidden",
//     // keep a 3D transform so we can safely not clear 'transform' immediately
//     transform: "translateZ(0)"
//   });
// }

// function unpinPanel(panel) {
//   // IMPORTANT: don't clear 'transform' immediately (prevents end-of-tween blink)
//   gsap.set(panel, {
//     clearProps: "position,top,left,width,height,willChange,zIndex,backfaceVisibility"
//   });
//   if (panel.__placeholder) {
//     panel.__placeholder.remove();
//     panel.__placeholder = null;
//   }
// }
...
//   // deterministic z-order during the slide
//   gsap.set(next, { autoAlpha: 1, zIndex: 10001, yPercent: 100 * dFactor });
//   if (prev) gsap.set(prev, { zIndex: 10000, yPercent: 0 });

//   const tl = gsap.timeline({
//     defaults: { duration: 0.9, ease: "power2.inOut" },
//     onComplete: () => {
//       // 1) Snap first, with clamp, so we don't overshoot at the bottom
//       const targetTop = Math.min(docMaxScroll(), Math.round(naturalTop(next)));
//       window.scrollTo(0, targetTop);

//       // 2) Defer unpin to next frame so painting settles before we drop fixed
//       requestAnimationFrame(() => {
//         if (prev) unpinPanel(prev);
//         unpinPanel(next);

//         // optional: after another tick, release the layer hint on the current
//         requestAnimationFrame(() => {
//           gsap.set(next, { clearProps: "willChange" });
//         });
...
//   // freeze scroll during tween to avoid drift
//   const freezeY = getScroll();
//   tl.eventCallback("onUpdate", () => window.scrollTo(0, freezeY));

//   // Slide transition: prev out, next in
//   if (prev) tl.to(prev, { yPercent: -100 * dFactor }, 0);
